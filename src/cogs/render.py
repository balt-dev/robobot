...
class Renderer:
    ...
    def recolor(
        ...
        if isinstance(color, str):
            if (isinstance(paint, bool) and paint) or force:
                ...
                overlay = np.tile(overlay, (math.ceil(overlay.shape[0] / sprite.shape[0]), math.ceil(overlay.shape[1] / sprite.shape[1]), 1))
    ...
    async def render(self, tiles: list[ProcessedTile], buf: BytesIO, ctx: RenderingContext) -> None:
        ...
        for tile in sorted(tiles, key=lambda til: til.z):
            ...
            sprite = cv2.resize(tile.sprite, (0, 0), fx=int(tile.scale[0]), fy=int(tile.scale[0]), interpolation=cv2.INTER_AREA)
            h, w = sprite.shape[:2]

            image_slice = slice(int(tile.y - h//2), int(tile.y + h//2)), slice(int(tile.x - w//2), int(tile.x + w//2))
            print(image_slice)
